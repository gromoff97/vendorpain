...
class GraphsClientTest {
    @Test
    fun `downloads csv with expected URL query and basic auth`() {
    ...
    @Test
    fun `token auth sends bearer authorization header`() {
        MockWebServer().use { server ->
            server.start()
            ...
            client(
                ...
                auth = Auth(method = "token", token = "secret-token"),
    ...
    private fun client(
        baseUrl: String,
        auth: Auth = Auth(method = "basic", username = "petrov.iv", password = "secret-password"),
